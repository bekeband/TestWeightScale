...
{
    /* Initialize USART */
    PLIB_USART_BaudRateSet(USART_ID_5, SYS_CLK_PeripheralFrequencyGet(CLK_BUS_PERIPHERAL_1), 9600);
    PLIB_USART_HandshakeModeSelect(USART_ID_5, USART_HANDSHAKE_MODE_FLOW_CONTROL);
    PLIB_USART_OperationModeSelect(USART_ID_5, USART_ENABLE_TX_RX_USED);
    PLIB_USART_LineControlModeSelect(USART_ID_5, USART_8N1);
    PLIB_USART_TransmitterEnable(USART_ID_5);
    PLIB_USART_TransmitterInterruptModeSelect(USART_ID_5, USART_TRANSMIT_FIFO_NOT_FULL);
    PLIB_USART_ReceiverEnable(USART_ID_5);
    PLIB_USART_ReceiverInterruptModeSelect(USART_ID_5, USART_RECEIVE_FIFO_ONE_CHAR);
    /* Initialize interrupts */
    /* Note: TX interrupt must be enabled when data is ready to be transmitted */
    /* The following code can be used to enable TX Interrupt */
    PLIB_INT_SourceEnable(INT_ID_0, INT_SOURCE_USART_5_TRANSMIT);
    PLIB_INT_SourceEnable(INT_ID_0, INT_SOURCE_USART_5_RECEIVE);
    PLIB_INT_SourceEnable(INT_ID_0, INT_SOURCE_USART_5_ERROR);
    PLIB_INT_VectorPrioritySet(INT_ID_0, INT_VECTOR_UART5, INT_PRIORITY_LEVEL1);
    PLIB_INT_VectorSubPrioritySet(INT_ID_0, INT_VECTOR_UART5, INT_SUBPRIORITY_LEVEL0);
...
uint8_t DRV_USART1_ReadByte(void)
{
   ...
   return (PLIB_USART_ReceiverByteReceive(USART_ID_5));
}
